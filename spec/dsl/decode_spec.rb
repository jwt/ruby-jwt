  ...
  describe '.decode!' do
    ...
    context 'when nothing but algorithm is defined' do
      it 'verifies a token and returns the data' do
        expect(jwt_class.decode!(encoded_payload, key: secret)).to eq([payload, { 'alg' => 'HS256' }])
      end
    end

    context 'when token is nil' do
      it 'raises JWT::DecodeError' do
        expect { jwt_class.decode!(nil) }.to raise_error(JWT::DecodeError, 'Provided token is not a String object')
      end
    end

    context 'when token is a 1' do
      it 'raises JWT::DecodeError' do
        expect { jwt_class.decode!(1) }.to raise_error(JWT::DecodeError, 'Provided token is not a String object')
      end
    end

    context 'when a decode_payload block manipulates the payload' do
      before do
        jwt_class.decode_payload do |raw_payload, _header, _signature|
      ...
      it 'uses the defined decode_payload to process the raw payload' do
        expect(jwt_class.decode!(encoded_payload).first['pay']).to eq('daol')
      end
    end

    context 'when block given' do
      it 'calls it with payload and header' do
        expect { |b| jwt_class.decode!(encoded_payload, &b) }.to yield_with_args(payload, { 'alg' => 'HS256' })
      end
    end

    context 'when given block returns something' do
      it 'returns what the block returned' do
        expect(jwt_class.decode!(encoded_payload) { '123' }).to eq('123')
      end
    end

    context 'when signing key is invalid' do
      it 'raises JWT::VerificationError' do
        expect { jwt_class.decode!(encoded_payload, key: 'invalid') }.to raise_error(JWT::VerificationError, 'Signature verification failed')
      end
    end

    context 'when algorithm is not matching the one in the token' do
      it 'raises JWT::VerificationError' do
        expect { jwt_class.decode!(encoded_payload, algorithms: ['HS512']) }.to raise_error(JWT::IncorrectAlgorithm, 'Expected a different algorithm')
      end
    end

    context 'when one of the given algorithms match' do
      it 'raises JWT::VerificationError' do
        expect(jwt_class.decode!(encoded_payload, algorithms: ['HS512', 'HS256'])).to eq([payload, { 'alg' => 'HS256' }])
      end
    end

    context 'when payload is invalid JSON' do
      before do
        jwt_class.encode_payload do |payload|
          Base64.urlsafe_encode64(payload.inspect, padding: false)
        end
      end

      let(:encoded_payload) { jwt_class.encode!(payload) }

      it 'raises JWT::DecodeError' do
        expect { jwt_class.decode!(encoded_payload) }.to raise_error(JWT::DecodeError, 'Invalid segment encoding')
      end
    end
    ...
    context 'when token is expired' do
      ...
      it 'allows token to be 30 seconds overdue' do
        expect { jwt_class.decode!(encoded_payload) }.to raise_error(JWT::ExpiredSignature, 'Signature has expired')
      end
    end

    context 'when expiration_leeway is set to 30 seconds' do
      before do
        jwt_class.expiration_leeway 30
      end
